...
# the docking pairs included in the mixed_docking_set.txt.
...
def log_time(interval, e):
	with open('mixed_timelog.txt','a') as outf:
...
# Input docking set
index_file = 'mixed_docking_set.txt'

# Fixed Output Directory
output_folder = 'mixed_docked_models'
...
for e in parse_dictionary_list(index_file):
	...
	# Parse this path differently depending on if this is
	# ModBase model (32 characters long) or a PDB structure.
	# Set chain appropriately.
	if len(receptor) == 32 or not "_" in receptor:  #modbase model IDs are 32 characters long
		receptor_file = os.path.join(modbase_model_dir, receptor+'.pdb')
		receptor_chain = '_'
	else:
		receptor_file, receptor_chain = receptor.split('_')
	
	if len(ligand) == 32 or not "_" in ligand:
		ligand_file = os.path.join(modbase_model_dir, ligand+'.pdb')
		ligand_chain = '_'
	else:
		ligand_file, ligand_chain = ligand.split('_')
