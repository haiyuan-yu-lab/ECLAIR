...
# the docking pairs included in the pdb_docking_set.txt.
...
def log_time(interval, e):
	with open('pdb_timelog.txt','a') as outf:
...
# Input docking set
index_file = 'pdb_docking_set.txt'
...
for e in parse_dictionary_list(index_file):
	...
	# and parse out their chains from the names
	...
	# Output results to the output_folder.
	#
	...
	command_queue.append(('wrapped_zdock.py %s %s -RC %s -LC %s -N 2000 -F --outdir %s' %(receptor_pdb, ligand_pdb, receptor_chain, ligand_chain, output_folder), e))
